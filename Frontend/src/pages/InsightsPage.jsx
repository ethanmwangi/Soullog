      ...
      {allInsights.length > 0 ? (
        <div className="insights-container">
          {filteredInsights.map((insight, index) => (
            <Link to={`/entry/${insight.entryId}`} key={insight.id || index} className="insight-card-link">
              <div 
                className={`insight-card ${insight.insight_type}`}
                style={{ animationDelay: `${index * 0.05}s` }} // Animation delay restored
              >
                ...
                <p className="insight-content-preview">{insight.content}</p>
